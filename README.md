# Finte Terraform for AWS Integration
...
4. **Policy** used to grant Finte access to read from the S3 bucket.

## Example Usage

The example below uses ref=main (which is appended in the URL), but it is recommended to use a specific tag version (i.e. ref=0.0.1) to avoid breaking changes. Go to the [release page](https://github.com/finte-public-ai/aws-terraform-finte-integration/releases) for a list of published versions.

You do not need to set any variables as the defaults will ensure your CUR 2.0 report is in the correct format and location. You
may wish to enable versioning of your S3 bucket, which can be done via the `enable_versioning` variable. We do NOT recommend
changing the CUR report 

```hcl
module "cur_s3_bucket" {
  source = "git::https://github.com/finte-public-ai/aws-terraform-finte-integration.git?ref=main"
  
  # no need to set any variables as the defaults
  # are configured to ensure your CUR 2.0 report is accessible by Finte
  
  # Uncomment the following if you need to change the default values for things such as the bucket name 
  # or would like to include tags
  # bucket_name = 'MyCustomName'
  # cur_report_name = 'MyCustomCurReportName'
  # tags = '{}'

  # Versioning is turned off by default since it will increase the size of your
  # S3 bucket. If you wish to enable versioning for your S3 bucket, simply 
...
# input in the Finte integration form
...
|------|-------------|------|---------|:--------:|
| <a name="input_bucket_name"></a> [bucket\_name](#input\_bucket\_name) | Name of the S3 bucket for storing CUR files | `string` | `"finte-cur"` | no |
...
| <a name="input_create_cur_access_policy"></a> [create\_cur\_access\_policy](#input\_create\_cur\_access\_policy) | Whether to create the FinTeCURAccessPolicy IAM policy used to grant the FinTeCrossAccountRole access to the S3 bucket which is storing the CUR 2.0 report. If false, you will need to create the policy manually. | `bool` | `true` | no |
| <a name="input_cross_account_role_name"></a> [cross\_account\_role\_name](#input\_cross\_account\_role\_name) | Name of the existing IAM role to attach the FinTeCURAccessPolicy to. If following our best practices, this will be the FinTeCrossAccountRole. | `string` | `"FinTeCrossAccountRole"` | no |
| <a name="input_cur_access_policy_name"></a> [cur\_access\_policy\_name](#input\_cur\_access\_policy\_name) | Name of the IAM policy used to grant the FinTeCrossAccountRole access to the S3 bucket which is storing the CUR 2.0 report. | `string` | `"FinTeCURAccessPolicy"` | no |
...
| <a name="input_cur_report_name"></a> [cur\_report\_name](#input\_cur\_report\_name) | Name of the Cost and Usage Report 2.0 export | `string` | `"FinteCostExportDaily"` | no |
